/*! \file   janus_ultrasound.c
 * \author Lorenzo Miniero <lorenzo@meetecho.com>
 * \copyright GNU General Public License v3
 * \brief  Janus Streaming plugin
 * \details  This is a ultrasound plugin for Janus, allowing WebRTC peers
 ...
 * 
 * -# on-demand ultrasound of pre-recorded media files (different
 * ultrasound context for each peer);
 * -# live ultrasound of pre-recorded media files (shared ultrasound
 * context for all peers attached to the stream);
 * -# live ultrasound of media generated by another tool (shared
 * ultrasound context for all peers attached to the stream).
 * 
 ...
 * A pre-filled configuration file is provided in \c conf/janus.plugin.ultrasound.cfg
...
type = rtp|live|ondemand|rtsp
       ...
       live = local file streamed live to multiple listeners
              (multiple listeners = same ultrasound context)
       ondemand = local file streamed on-demand to a single listener
                  (multiple listeners = different ultrasound contexts)
...
static gboolean janus_ultrasound_is_keyframe(gint codec, char* buffer, int len);

typedef enum janus_ultrasound_type {
	janus_ultrasound_type_none = 0,
	janus_ultrasound_type_live,
	janus_ultrasound_type_on_demand,
} janus_ultrasound_type;

typedef enum janus_ultrasound_source {
	janus_ultrasound_source_none = 0,
	janus_ultrasound_source_file,
	janus_ultrasound_source_rtp,
} janus_ultrasound_source;
...
typedef struct janus_ultrasound_rtp_source {
	gint audio_port;
...
} janus_ultrasound_rtp_source;

typedef struct janus_ultrasound_file_source {
	char *filename;
} janus_ultrasound_file_source;

#define JANUS_ULTRASOUND_VP8		0
#define JANUS_ULTRASOUND_H264	1
#define JANUS_ULTRASOUND_VP9		2
...
typedef struct janus_ultrasound_mountpoint {
	...
	gboolean enabled;
	gboolean active;
	janus_ultrasound_type ultrasound_type;
	janus_ultrasound_source ultrasound_source;
...
janus_ultrasound_mountpoint *janus_ultrasound_create_file_source(
...
janus_ultrasound_mountpoint *janus_ultrasound_create_rtsp_source(
...
int janus_ultrasound_init(janus_callbacks *callback, const char *config_path) {
	...
	/* Read configuration */
	char filename[255];
	g_snprintf(filename, 255, "%s/%s.cfg", config_path, JANUS_ULTRASOUND_PACKAGE);
		...
		while(cl != NULL) {
			...
			if(!strcasecmp(type->value, "rtp")) {
				...
				if(id == NULL || id->value == NULL) {
					...
					janus_mutex_lock(&mountpoints_mutex);
					janus_ultrasound_mountpoint *mp = g_hash_table_lookup(mountpoints, GINT_TO_POINTER(atol(id->value)));
					janus_mutex_unlock(&mountpoints_mutex);
					if(mp != NULL) {
				...
				janus_ultrasound_mountpoint *mp = NULL;
				if((mp = janus_ultrasound_create_rtp_source(
				...
				if(!doaudio || dovideo) {
					JANUS_LOG(LOG_ERR, "Can't add 'live' stream '%s', we only support audio file ultrasound right now...\n", cat->name);
					cl = cl->next;
					continue;
				}
				...
				janus_ultrasound_mountpoint *mp = NULL;
				if((mp = janus_ultrasound_create_file_source(
						(id && id->value) ? atol(id->value) : 0,
						(char *)cat->name,
						desc ? (char *)desc->value : NULL,
						(char *)file->value,
				...
				/* TODO We should support something more than raw a-Law and mu-Law streams... */
				if(!doaudio || dovideo) {
					JANUS_LOG(LOG_ERR, "Can't add 'ondemand' stream '%s', we only support audio file ultrasound right now...\n", cat->name);
					cl = cl->next;
					continue;
				}
				if(!strstr(file->value, ".alaw") && !strstr(file->value, ".mulaw")) {
				...
				janus_ultrasound_mountpoint *mp = NULL;
				if((mp = janus_ultrasound_create_rtsp_source(
						(id && id->value) ? atol(id->value) : 0,
						(char *)cat->name,
						desc ? (char *)desc->value : NULL,
	...
	while (g_hash_table_iter_next(&iter, NULL, &value)) {
		...
		JANUS_LOG(LOG_VERB, "  ::: [%"SCNu64"][%s] %s (%s, %s, %s, pin: %s)\n", mp->id, mp->name, mp->description,
			mp->ultrasound_type == janus_ultrasound_type_live ? "live" : "on demand",
			mp->ultrasound_source == janus_ultrasound_source_rtp ? "RTP source" : "file source",
...
struct janus_plugin_result *janus_ultrasound_handle_message(janus_plugin_session *handle, char *transaction, char *message, char *sdp_type, char *sdp) {
	...
	if(!strcasecmp(request_text, "list")) {
		...
		while (g_hash_table_iter_next(&iter, NULL, &value)) {
			...
			json_object_set_new(ml, "type", json_string(mp->ultrasound_type == janus_ultrasound_type_live ? "live" : "on demand"));
			if(mp->ultrasound_source == janus_ultrasound_source_rtp) {
				janus_ultrasound_rtp_source *source = mp->source;
		...
		json_object_set_new(response, "ultrasound", json_string("list"));
		...
		janus_mutex_lock(&mountpoints_mutex);
		janus_ultrasound_mountpoint *mp = g_hash_table_lookup(mountpoints, GINT_TO_POINTER(id_value));
		if(mp == NULL) {
			janus_mutex_unlock(&mountpoints_mutex);
		...
		/* Send info back */
		response = json_object();
		json_object_set_new(response, "ultrasound", json_string("info"));
		...
		if(!strcasecmp(type_text, "rtp")) {
			...
			if(!doaudio && !dovideo) {
				...
				error_code = JANUS_ULTRASOUND_ERROR_CANT_CREATE;
			...
			JANUS_LOG(LOG_VERB, "Audio %s, Video %s\n", doaudio ? "enabled" : "NOT enabled", dovideo ? "enabled" : "NOT enabled");
			mp = janus_ultrasound_create_rtp_source(
			...
			/* TODO We should support something more than raw a-Law and mu-Law streams... */
			if(!doaudio || dovideo) {
				JANUS_LOG(LOG_ERR, "Can't add 'live' stream, we only support audio file ultrasound right now...\n");
				error_code = JANUS_ULTRASOUND_ERROR_CANT_CREATE;
				g_snprintf(error_cause, 512, "Can't add 'live' stream, we only support audio file ultrasound right now...");
				goto error;
			}
			...
			mp = janus_ultrasound_create_file_source(
					id ? json_integer_value(id) : 0,
					name ? (char *)json_string_value(name) : NULL,
					desc ? (char *)json_string_value(desc) : NULL,
					filename,
			...
			/* TODO We should support something more than raw a-Law and mu-Law streams... */
			if(!doaudio || dovideo) {
				JANUS_LOG(LOG_ERR, "Can't add 'ondemand' stream, we only support audio file ultrasound right now...\n");
				error_code = JANUS_ULTRASOUND_ERROR_CANT_CREATE;
				g_snprintf(error_cause, 512, "Can't add 'ondemand' stream, we only support audio file ultrasound right now...");
				goto error;
			}
			char *filename = (char *)json_string_value(file);
			if(!strstr(filename, ".alaw") && !strstr(filename, ".mulaw")) {
			...
			mp = janus_ultrasound_create_rtsp_source(
					id ? json_integer_value(id) : 0,
					name ? (char *)json_string_value(name) : NULL,
					desc ? (char *)json_string_value(desc) : NULL,
		...
		if(save) {
			...
			if(!strcasecmp(type_text, "rtp")) {
				...
				janus_ultrasound_file_source *source = mp->source;
			...
			janus_config_save(config, config_folder, JANUS_ULTRASOUND_PACKAGE);
			janus_mutex_unlock(&config_mutex);
		}
		/* Send info back */
		response = json_object();
		json_object_set_new(response, "ultrasound", json_string("created"));
		...
		/* Send info back */
		response = json_object();
		json_object_set_new(response, "ultrasound", json_string("destroyed"));
		...
		if(mp->ultrasound_type != janus_ultrasound_type_live || mp->ultrasound_source != janus_ultrasound_source_rtp) {
			janus_mutex_unlock(&mountpoints_mutex);
		...
		if(!strcasecmp(action_text, "start")) {
			...
			response = json_object();
			json_object_set_new(response, "ultrasound", json_string("ok"));
			goto plugin_response;
...
void janus_ultrasound_setup_media(janus_plugin_session *handle) {
	...
	if(mountpoint->ultrasound_source == janus_ultrasound_source_rtp) {
		janus_ultrasound_rtp_source *source = mountpoint->source;
		if(source->keyframe.enabled) {
...
static void *janus_ultrasound_handler(void *data) {
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(!strcasecmp(request_text, "watch")) {
			...
			if(mp->ultrasound_type == janus_ultrasound_type_on_demand) {
				GError *error = NULL;
				g_thread_try_new(session->mountpoint->name, &janus_ultrasound_ondemand_thread, session, &error);
				if(error != NULL) {
...
#define sdp_d_template \
		...
		"a=sctpmap:5000 webrtc-datachannel 16\r\n"
			...
			janus_ultrasound_mountpoint *oldmp = session->mountpoint;
			if(oldmp == NULL) {
			...
			if(oldmp->ultrasound_type != janus_ultrasound_type_live || 
					oldmp->ultrasound_source != janus_ultrasound_source_rtp) {
				JANUS_LOG(LOG_VERB, "Can't switch: not on a live RTP mountpoint\n");
				error_code = JANUS_ULTRASOUND_ERROR_CANT_SWITCH;
			...
			if(mp->ultrasound_type != janus_ultrasound_type_live || 
					mp->ultrasound_source != janus_ultrasound_source_rtp) {
				janus_mutex_unlock(&mountpoints_mutex);
			...
			/* Done */
			result = json_object();
			json_object_set_new(result, "ultrasound", json_string("event"));
...
/* Helpers to destroy a ultrasound mountpoint. */
static void janus_ultrasound_rtp_source_free(janus_ultrasound_rtp_source *source) {
...
static void janus_ultrasound_file_source_free(janus_ultrasound_file_source *source) {
...
janus_ultrasound_mountpoint *janus_ultrasound_create_rtp_source(
	...
	int audio_fd = -1;
	if(doaudio) {
		audio_fd = janus_ultrasound_create_fd(aport, amcast ? inet_addr(amcast) : INADDR_ANY,
	...
	int video_fd = -1;
	if(dovideo) {
		video_fd = janus_ultrasound_create_fd(vport, vmcast ? inet_addr(vmcast) : INADDR_ANY,
	...
	live_rtp->ultrasound_type = janus_ultrasound_type_live;
	live_rtp->ultrasound_source = janus_ultrasound_source_rtp;
	janus_ultrasound_rtp_source *live_rtp_source = g_malloc0(sizeof(janus_ultrasound_rtp_source));
	...
	live_rtp->source_destroy = (GDestroyNotify) janus_ultrasound_rtp_source_free;
	...
	if(dovideo) {
		if(strstr(vrtpmap, "vp8") || strstr(vrtpmap, "VP8"))
			...
			live_rtp->codecs.video_codec = JANUS_ULTRASOUND_VP9;
		else if(strstr(vrtpmap, "h264") || strstr(vrtpmap, "H264"))
			live_rtp->codecs.video_codec = JANUS_ULTRASOUND_H264;
	}
...
janus_ultrasound_mountpoint *janus_ultrasound_create_file_source(
	...
	/* FIXME We don't support video ultrasound from file yet */
	if(!doaudio || dovideo) {
		JANUS_LOG(LOG_ERR, "Can't add 'file' stream, we only support audio file ultrasound right now...\n");
		janus_mutex_unlock(&mountpoints_mutex);
		return NULL;
	}
	/* TODO We should support something more than raw a-Law and mu-Law streams... */
	...
	janus_ultrasound_mountpoint *file_source = g_malloc0(sizeof(janus_ultrasound_mountpoint));
	if(file_source == NULL) {
		JANUS_LOG(LOG_FATAL, "Memory error!\n");
		janus_mutex_unlock(&mountpoints_mutex);
		return NULL;
	...
	file_source->ultrasound_type = live ? janus_ultrasound_type_live : janus_ultrasound_type_on_demand;
	file_source->ultrasound_source = janus_ultrasound_source_file;
	janus_ultrasound_file_source *file_source_source = g_malloc0(sizeof(janus_ultrasound_file_source));
	...
	file_source->source_destroy = (GDestroyNotify) janus_ultrasound_file_source_free;
	...
	if(live) {
		GError *error = NULL;
		g_thread_try_new(file_source->name, &janus_ultrasound_filesource_thread, file_source, &error);
		if(error != NULL) {
...
#ifdef HAVE_LIBCURL		
typedef struct janus_ultrasound_buffer {
	char *buffer;
	size_t size;
} janus_ultrasound_buffer;

static size_t janus_ultrasound_rtsp_curl_callback(void *payload, size_t size, size_t nmemb, void *data) {
	size_t realsize = size * nmemb;
	janus_ultrasound_buffer *buf = (struct janus_ultrasound_buffer *)data;
...
static int janus_ultrasound_rtsp_parse_sdp(const char* buffer, const char* name, const char* media, int* pt, char* transport, char* rtpmap, char* fmtp, char* control) {
	int port=-1;
	char pattern[256];
	...
	int fd = janus_ultrasound_create_fd(port, mcast, media, media, name);
	if(fd < 0) {
		return -1;
	}
	struct sockaddr_in address;	
...
janus_ultrasound_mountpoint *janus_ultrasound_create_rtsp_source(
	...
	curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, janus_ultrasound_rtsp_curl_callback);		
	curl_easy_setopt(curl, CURLOPT_WRITEDATA, &data);
	curl_easy_setopt(curl, CURLOPT_HEADERFUNCTION, janus_ultrasound_rtsp_curl_callback);		
	...
	char uri[1024];
	char transport[1024];
	int video_fd = janus_ultrasound_rtsp_parse_sdp(data.buffer, name, "video", &vpt, transport, vrtpmap, vfmtp, vcontrol);
	if(video_fd != -1) {
	...
	char afmtp[2048];
	char acontrol[2048];
	int audio_fd = janus_ultrasound_rtsp_parse_sdp(data.buffer, name, "audio", &apt, transport, artpmap, afmtp, acontrol);
	if(audio_fd != -1) {
	...
	janus_ultrasound_mountpoint *live_rtsp = g_malloc0(sizeof(janus_ultrasound_mountpoint));
	if(live_rtsp == NULL) {
		JANUS_LOG(LOG_FATAL, "Memory error!\n");
		g_free(description);
		g_free(sourcename);
	...
	live_rtsp->ultrasound_type = janus_ultrasound_type_live;
	live_rtsp->ultrasound_source = janus_ultrasound_source_rtp;
	janus_ultrasound_rtp_source *live_rtsp_source = g_malloc0(sizeof(janus_ultrasound_rtp_source));
	...
	live_rtsp->source_destroy = (GDestroyNotify) janus_ultrasound_rtp_source_free;
	...
	janus_mutex_unlock(&mountpoints_mutex);	
	GError *error = NULL;
	g_thread_try_new(live_rtsp->name, &janus_ultrasound_relay_thread, live_rtsp, &error);	
	if(error != NULL) {
		JANUS_LOG(LOG_ERR, "Got error %d (%s) trying to launch the RTSP thread...\n", error->code, error->message ? error->message : "??");
		janus_ultrasound_mountpoint_free(live_rtsp);
		return NULL;	
	}				
	/* Send an RTSP PLAY */
	g_free(data.buffer);
...
static void *janus_ultrasound_ondemand_thread(void *data) {
	...
	if(mountpoint->ultrasound_source != janus_ultrasound_source_file) {
		JANUS_LOG(LOG_ERR, "[%s] Not an file source mountpoint!\n", mountpoint->name);
		g_thread_unref(g_thread_self());
		return NULL;
	}
	if(mountpoint->ultrasound_type != janus_ultrasound_type_on_demand) {
		JANUS_LOG(LOG_ERR, "[%s] Not an on-demand file source mountpoint!\n", mountpoint->name);
		g_thread_unref(g_thread_self());
		return NULL;
	}
	janus_ultrasound_file_source *source = mountpoint->source;
	...
	while(!g_atomic_int_get(&stopping) && !mountpoint->destroyed && !session->stopping && !session->destroyed) {
		...
		/* Go! */
		janus_ultrasound_relay_rtp_packet(session, &packet);
		/* Update header */
		seq++;
		header->seq_number = htons(seq);
		ts += 160;
...
static void *janus_ultrasound_filesource_thread(void *data) {
	...
	if(mountpoint->ultrasound_type != janus_ultrasound_type_live) {
				...
				if(audio_fd != -1 && fds[i].fd == audio_fd) {
					...
					if(source->keyframe.enabled) {
						if(source->keyframe.temp_ts > 0 && ntohl(rtp->timestamp) != source->keyframe.temp_ts) {
							...
							janus_ultrasound_rtp_relay_packet *pkt = g_malloc0(sizeof(janus_ultrasound_rtp_relay_packet));
							...
							janus_mutex_unlock(&source->keyframe.mutex);
						} else if(janus_ultrasound_is_keyframe(mountpoint->codecs.video_codec, buffer, bytes)) {
...
static gboolean janus_ultrasound_is_keyframe(gint codec, char* buffer, int len) {
	if(codec == JANUS_ULTRASOUND_VP8) {
		...
		return FALSE;
	} else if(codec == JANUS_ULTRASOUND_H264) {
		/* Parse RTP header first */
		rtp_header *header = (rtp_header *)buffer;
		guint32 timestamp = ntohl(header->timestamp);
		guint16 seq = ntohs(header->seq_number);
